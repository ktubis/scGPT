...
import sys
import load_ds
from model_wrappers.model_wrappers import ModelLoader, move_optimizer_params_to_cuda
import random
import torch
import numpy as np
import os
...
def get_data_loaders(ds_loader, vocab, n_input_bins, test_data, n_hvg, model_task):
    ...
    adata_train = preprocess_data(adata_train, preprocessor, vocab, batch_key)
    adata_test = preprocess_data(adata_test, preprocessor, vocab, batch_key)
...
def main():
    ...
    parser.add_argument("--train_data", type=str, default="pc", help="Which dataset to use for the train data.")
    parser.add_argument("--max_seq_len", type=int, default=0, help="Maximum sequence length")
    ...
    if args.hyperparam_search_config:
        ...
        if not args.inference:
            ...
            if args.celltype_list:
                celltype_list = [s.replace(' ', '_') for s in args.celltype_list]
                cell_list = "_".join(celltype_list)
                test_kwargs["celltype_list"] = celltype_list
